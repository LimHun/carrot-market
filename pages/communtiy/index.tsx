    ...
    return (
        <Layout title="동내생활" hasTabBar>
            <div className="py-2 px-4 space-y-8">
                {data?.posts?.map((post) => (
                    <Link key={post.id} href={`/communtiy/${post.id}`}>
                ...
                <button
                    onClick={onClick}
                    className="hover:bg-orange-500 cursor-pointer transition-colors fixed bottom-28 right-5 bg-orange-400 rounded-full text-white p-4 shadow-xl"
                >
                    <svg
                        className="w-6 h-6"
                        fill="none"
